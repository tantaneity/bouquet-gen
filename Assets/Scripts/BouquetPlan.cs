...
internal static class BouquetPlan
{
    ...
    public static List<Stalk> Build(BouquetSettings settings, BouquetPalette palette, Vector3 bind)
    {
        ...
        for (int role = 0; role < RoleCount; role++)
        {
            ...
            for (int slot = 0; slot < band.countHigh; slot++, index++)
            {
                ...
                Stalk stalk = Grow(settings, palette, bind, index, (Role)role, target * growth, growth);

                if (stalk.species.IsBloom() && role <= (int)Role.Medium)
                {
        ...
        StalkClearance.Resolve(plan, (stalk, target) => Grow(settings, palette, bind, stalk.index, stalk.role, target, stalk.growth));
        return plan;
    }

    private static Stalk Grow(BouquetSettings settings, BouquetPalette palette, Vector3 bind,
    ...
    private static Vector3 TargetFor(BouquetSettings settings, Role role, Band band, int slot, float count, int index)
    {
        ...
        if (role == Role.Dominant)
        {
            ...
            Vector3 jitter = new Vector3(
                (BouquetHash.Unit(index, 15, seed) - 0.5f) * 0.14f,
                (BouquetHash.Unit(index, 16, seed) - 0.5f) * 0.12f,
                (BouquetHash.Unit(index, 17, seed) - 0.5f) * 0.10f);
            Vector3 offset = (staged + jitter) * scale;
            return new Vector3(offset.x, offset.y, 0.0f) + TowardViewer * offset.z;
        }

        float across = (slot + 0.15f + 0.70f * BouquetHash.Unit(index, 0, seed)) / count - 0.5f;
        ...
        float radius = Mathf.Lerp(band.radiusLow, band.radiusHigh, BouquetHash.Unit(index, 1, seed))
                     * flatten * (1.0f + settings.asymmetry * bias) * settings.spreadGain;
        float height = Mathf.Lerp(band.heightLow, band.heightHigh, BouquetHash.Unit(index, 4, seed));
        ...
        Vector3 bind, List<GrownTip> bloomTips, int index)
    {
        ...
        for (int slot = 0; slot < band.countHigh; slot++, index++)
        {
            int a = Mathf.FloorToInt(BouquetHash.Unit(index, 18, settings.seed) * (bloomTips.Count - 0.001f));
            int b = (a + 1 + Mathf.FloorToInt(BouquetHash.Unit(index, 19, settings.seed) * (bloomTips.Count - 1.001f))) % bloomTips.Count;
            ...
            Vector3 gap = Vector3.Lerp(bloomTips[a].tip, bloomTips[b].tip, 0.35f + 0.30f * BouquetHash.Unit(index, 23, settings.seed));
            gap -= bind;
            gap += (new Vector3(
                (BouquetHash.Unit(index, 24, settings.seed) - 0.5f) * 0.16f,
                (BouquetHash.Unit(index, 25, settings.seed) - 0.5f) * 0.20f + 0.06f,
                0.0f)
                - TowardViewer * (0.10f + 0.18f * BouquetHash.Unit(index, 26, settings.seed))) * settings.stemLength;

            plan.Add(Grow(settings, palette, bind, index, Role.Filler, gap * growth, growth));
        }
    }
        ...
        int index, Role role, Band band, Vector3 target)
    {
        ...
        Species species = RoleSpecies[(int)role * 4 + Mathf.FloorToInt(BouquetHash.Unit(index, 2, seed) * 3.999f)];
        float headSize = settings.headScale * Mathf.Lerp(band.scaleLow, band.scaleHigh, BouquetHash.Unit(index, 3, seed));
        ...
        Vector3 anchor = bind
            ...
            + Vector3.up * (BouquetHash.Unit(index, 6, seed) - 0.5f) * settings.tieHeight;
        ...
        float bend = (BouquetHash.Unit(index, 8, seed) - 0.5f) * settings.sideBend;
        float curve = settings.outwardCurve * (0.50f + 1.00f * BouquetHash.Unit(index, 9, seed));

        Vector3 handleLow = anchor + Vector3.up * reach * curve * 0.48f;
        Vector3 handleHigh = tip - direction * reach * 0.34f + lateral * bend * reach;

        float cutLength = settings.cutLength * (0.5f + 0.5f * BouquetHash.Unit(index, 10, seed));
        Vector3 cutEnd = anchor + Vector3.Normalize(new Vector3(-direction.x, -1.05f, -direction.z)) * cutLength;

        int bloomSlot = Mathf.FloorToInt(BouquetHash.Unit(index, 11, seed) * 2.999f);
        Color bloom = palette.Vary(palette.Bloom(bloomSlot),
            BouquetHash.Unit(index, 12, seed), BouquetHash.Unit(index, 13, seed), settings.colourVariation);
        ...
        return new Stalk
        {
            ...
            roll = BouquetHash.Unit(index, 14, seed) * Mathf.PI * 2.0f,
            index = index,
            role = role,
            target = target
        };
